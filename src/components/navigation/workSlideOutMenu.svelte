...
<div
  class={`fixed top-0 left-0 w-full h-full flex transform transition-transform duration-300 z-50 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
>
  <div
    class="flex-1 bg-white flex flex-col items-center justify-center z-50 mobile:w-full"
  >
    <div class="p-5 text-left w-full relative ml-12 mobile:ml-0">
      {#each works as work}
        <p
          ...
          class="mb-4 cursor-pointer transition-colors duration-300 ${selectedWork ===
          work.title
  ...
  <div
    class={`flex-1 transition-colors duration-300 z-40 ${isTransitioning ? "bg-white" : ""} ${isWhiteBackground ? "bg-white" : ""} ${isClosing ? "bg-opacity-0" : ""} mobile:hidden`}
  >
    <!-- Right side content -->
  </div>
</div>
